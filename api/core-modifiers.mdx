---
title: "Core Generator Modifiers on Generator<T>"
...
Casts the generated value to `TOut`. Returns a `LiteralGenerator` wrapping the result of a single `Generate()` call. Returns `null` if the runtime cast is not possible.
